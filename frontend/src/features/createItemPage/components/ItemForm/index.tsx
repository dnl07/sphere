...
const FIELDS: {
    name: keyof CreateClothingItemRequest
    label: string
    placeholder: string
    type?: string
    required?: boolean
        ...
        if (type === "checkbox") {
            ...
            return (
                <FormDropdown
                    ...
                    setValue={(value: string) => updateRequest({ [name]: value })}
                />
            )
        } else {
            return (
        ...
        <Card title="Details">
            <div className="flex flex-col gap-6">
                {FIELDS.map(({ name, label, placeholder, type, required }) => (
                    <div
                        className={`flex gap-2 ${type === "checkbox" ? "flex-row items-center justify-between" : "flex-col"}`}
                        key={name}
                    >
