...
const FilterIconButton = ({ onFilter }: Props) => {
    ...
    return (
        <div className={`fixed bottom-1 md:bottom-3 z-30 max-w-6xl w-full flex px-3 pointer-events-none`}>
            ...
            >
                <div
                    className={`flex justify-center items-center rounded-lg text-white text-xl border-border-white border-2 h-15 p-2 transition-all ease-in-out duration-200  ${isExpanded ? "w-full" : "w-15"}`}
                >
                    {isExpanded ? (
                        <input
                            type="text"
                ...
                <button
                    className="text-white flex justify-center rounded-lg items-center border-border-white border-2 h-15 p-2 aspect-square cursor-pointer"
