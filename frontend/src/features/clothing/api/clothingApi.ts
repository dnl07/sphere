...
import type {
    ...
    GetClothingParams,
    UpdateClothingItemByIdRequest,
} from "./clothingApi.types"
...
// GET ALL
export async function getClothingItems(params: GetClothingParams = {}): Promise<GetClothingItemsResponse> {
    const response = await api.clothing.getAll(params)
    return response.data
}

// GET count
export async function getClothingCount(params: GetClothingParams = {}): Promise<number> {
    const response = await api.clothing.getCount(params)
    return response.data.count
}

// POST /clothing request types
export async function createClothingItem(request: CreateClothingItemRequest): Promise<ClothingItemDto> {
...
export async function updateClothingItemById(
    ...
    if (params.isArchived !== undefined && params.isArchived !== null)
        formData.append("IsArchived", String(params.isArchived))
